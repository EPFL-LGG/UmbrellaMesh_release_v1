...
void ElasticRod_T<Real_>::setRestConfiguration(const std::vector<Pt3_T<Real_>> &points) {
    ...
    {
        // Choose orthogonal reference directors (d1, d2) spanning the plane
        // orthogonal to the centerline's tangent vectors.

        // For curves in the x-y plane, pick rest director d2 = +z.
        // Otherwise, pick it arbitrarily.
        {
            auto zcmp = [](const Pt3 &a, const Pt3 &b) { return a[2] < b[2]; };
            Real_ zheight = (*std::max_element(points.begin(), points.end(), zcmp))[2]
                          - (*std::min_element(points.begin(), points.end(), zcmp))[2];
            if (zheight < 1e-12) {
                std::vector<Vec3> rest_d2;
                rest_d2.reserve(ne);
                rest_d2.emplace_back(Vec3(0, 0, 1));
                // Parallel transport this reference vector to the remaining rod edges
                for (size_t i = 1; i < ne; ++i) {
                    rest_d2.emplace_back(
                        parallelTransportNormalized<Real_>(unit_tangents[i - 1], unit_tangents[i],
                                                           rest_d2.back()));
                }

                // Compute the orthogonal reference director (d1) for each edge
                m_restDirectors.clear();
                m_restDirectors.reserve(ne);
                for (size_t j = 0; j < ne; ++j) {
                    m_restDirectors.emplace_back(rest_d2[j].cross(unit_tangents[j]),
                                                 rest_d2[j]);
                }
            } else {
...
void ElasticRod_T<Real_>::DeformedState::update(const std::vector<Pt3_T<Real_>> &points, const std::vector<Real_> &thetas) {
    ...
    per_corner_kappa[     0].setZero();
    per_corner_kappa[nv - 1].setZero();
...
Real_ ElasticRod_T<Real_>::thetaForMaterialFrameD2(Vec3_T<Real_> d2, const Vec3_T<Real_> &eNew, size_t j, bool spatialCoherence) const {
    ...
    if (std::abs(stripAutoDiff(parallelComp)) > 1e-8) {
        std::cerr << "WARNING: non-perpendicular normal!" << " cosine: "<<std::abs(stripAutoDiff(parallelComp)) <<std::endl;
...
Real_ ElasticRod_T<Real_>::energyStretch() const {
    ...
    Real_ result = 0;
    for (size_t j = 0; j < ne; ++j) {
        if (m_restLen[j] < 0) return safe_numeric_limits<Real_>::max(); // Infinite energy for rest state inversion.
...
Real_ ElasticRod_T<Real_>::energyBend() const {
    ...
    if (m_bendingEnergyType == BendingEnergyType::Bergou2010) {
        for (size_t i = 1; i < nv - 1; ++i) {
            ...
            if (libar2 < 0) return safe_numeric_limits<Real_>::max(); // Infinite energy for rest state inversion.
        ...
        for (size_t i = 1; i < nv - 1; ++i) {
            ...
            if (inv_libar2 < 0) return safe_numeric_limits<Real_>::max(); // Infinite energy for rest state inversion.
...
typename ElasticRod_T<Real_>::Gradient ElasticRod_T<Real_>::gradEnergyStretch(bool variableDesignParameters, bool designParameterOnly, const StencilMask &sm)  const {
    // d 0.5 sum_j ks^j (l^j / rl^j - 1)^2 rl^j
    //     = sum_j ks^j (l^j / rl^j - 1) * dl^j
    // d l^j = +/- tangent if x is +/- endpoint
    const size_t ne = numEdges();
    Gradient result(*this, variableDesignParameters);
    ...
    for (size_t j = 0; j < ne; ++j) {
        ...
        if (variableDesignParameters && m_designParameterConfig.restLen)
            result.gradDesignParameters(j) = density(j) * m_stretchingStiffness[j] * 0.5 * (1.0 - fracLen * fracLen);
    }

    return result;
}

// Compute the derivative of per_corner_kappa[i](k, adj_edge) with respect to edge vectors e_{i-1} and e_i.
// The deriative wrt e_{i - 1} is returned in the first column, and the deriative wrt e_i is returned in the second column.
template<typename Real_, class DC>
Eigen::Matrix<Real_, 3, 2> compute_d_kappa_i_k_j_de(bool updatedSource, const DC &dc, int i, int k, int adj_edge) {
...
typename ElasticRod_T<Real_>::Gradient ElasticRod_T<Real_>::gradEnergyBend(bool updatedSource, bool variableDesignParameters, bool designParameterOnly, const StencilMask &sm) const {
    const size_t nv = numVertices();
    Gradient result(*this, variableDesignParameters);
    ...
    for (size_t i = 1; i < nv - 1; ++i) {
        ...
        const size_t rk_design_var = i + m_designParameterConfig.restLen * m_restLen.size() - 1;
        ...
        for (size_t adj_edge = 0; adj_edge < 2; ++adj_edge) { // 0 ==> i - 1, 1 ==> i
            ...
            if (m_bendingEnergyType == BendingEnergyType::Bergou2010) {
                ...
                if (variableDesignParameters) {
                    if (m_designParameterConfig.restLen) {
                        Real_ contrib = m_bendingStiffness[i].lambda_1 * kappaDiff[0] * kappaDiff[0]
                                      + m_bendingStiffness[i].lambda_2 * kappaDiff[1] * kappaDiff[1];
                        result.gradDesignParameters(j) -= inv_2libar * inv_2libar * contrib;
                    }

                    if (m_designParameterConfig.restKappa) {
                        Real_ contrib =  m_bendingStiffness[i].lambda_1 * kappaDiff[0];
                        // There is no multiply by 2 because this is inside the adj_edge loop and this operation will happen twice.
                        result.gradDesignParameters(rk_design_var) -= inv_2libar * contrib;
                    }
                    if (designParameterOnly) continue;
                }
                ...
                if (variableDesignParameters) {
                    if (m_designParameterConfig.restLen) {
                        ...
                        result.gradDesignParameters(j     ) += (inv_2libar * inv_2libar * inv_2libar) * (m_restLen[jother] - m_restLen[j]) * contrib;
                        result.gradDesignParameters(jother) -= (inv_2libar * inv_2libar * inv_2libar) * (                2 * m_restLen[j]) * contrib;
                    }

                    if (m_designParameterConfig.restKappa) {
                        Real_ contrib = m_bendingStiffness[i].lambda_1 * kappaDiff[0];
                        result.gradDesignParameters(rk_design_var) -= (inv_2libar * inv_2libar) * (2 * m_restLen[j]) * contrib;
                    }
                    if (designParameterOnly) continue;
                }
            ...
            for (size_t k = 0; k < 2; ++k)
                dE_de += dE_dkappa_k_j[k] * compute_d_kappa_i_k_j_de<Real_>(updatedSource, dc, i, k, adj_edge);
        }
...
// Compute the derivative of a vertex's twist (m) with respect to its incident
// edge vectors e_{i-1} and e_i.
template<typename Real_, class DC>
void compute_d_twist_d_e(bool updatedSource, const DC &dc, int i, Eigen::Ref<Vec3_T<Real_>> d_m_de_im1, Eigen::Ref<Vec3_T<Real_>> d_m_de_i) {
    using Vec3 = Vec3_T<Real_>;
    // Compute centerline position dependence
    const Real_ inv_len_i   = 1.0 / dc.len[i    ],
                inv_len_im1 = 1.0 / dc.len[i - 1];
    d_m_de_i   = 0.5 * inv_len_i   * dc.kb[i],
...
template<typename Real_>
template<class StencilMask>
typename ElasticRod_T<Real_>::Gradient ElasticRod_T<Real_>::gradEnergyTwist(bool updatedSource, bool variableDesignParameters, bool designParameterOnly, const StencilMask &sm) const {
    const size_t nv = numVertices();
    Gradient result(*this, variableDesignParameters);
    ...
    for (size_t i = 1; i < nv - 1; ++i) {
        ...
        const Real_ twistDiff = (dc.theta(i) - dc.theta(i - 1) + dc.referenceTwist[i] - m_restTwist[i]);

        if (variableDesignParameters && m_designParameterConfig.restLen) {
            Real_ dE_dljbar = -0.25 * m_twistingStiffness[i] * inv_libar * inv_libar * twistDiff * twistDiff;
            result.gradDesignParameters(i - 1) += dE_dljbar;
            result.gradDesignParameters(i    ) += dE_dljbar;
            if (designParameterOnly) continue;
        }

        const Real_ dE_dm = inv_libar * m_twistingStiffness[i] * twistDiff;
        Vec3 d_m_de_im1, d_m_de_i;
        compute_d_twist_d_e<Real_>(updatedSource, dc, i, d_m_de_im1, d_m_de_i);
...
template<typename Real_>
template<class StencilMask>
typename ElasticRod_T<Real_>::Gradient ElasticRod_T<Real_>::gradEnergy(bool updatedSource, bool variableDesignParameters, bool designParameterOnly, const StencilMask &sm) const {
    auto result = gradEnergyStretch(             variableDesignParameters, designParameterOnly, sm);
    result +=     gradEnergyBend(updatedSource,  variableDesignParameters, designParameterOnly, sm);
    result +=     gradEnergyTwist(updatedSource, variableDesignParameters, designParameterOnly, sm);
    return result;
}
...
// configuration).
template<typename Real_>
size_t ElasticRod_T<Real_>::hessianNNZ(bool variableDesignParameters) const {
    const size_t nv = numVertices(), ne = numEdges();
    ...
    if (variableDesignParameters) {
        ...
        if (m_designParameterConfig.restKappa) {
            result += 3 *                         // x-restkappaVar consists of 3x1 blocks.
            (3 * numRestKappaVars());             //    The vertex contributes 3 blocks for the two adjacent edges
            result += numRestKappaVars() * 2;     // theta-restKappa is two diagonals
            result += numRestKappaVars() * 2;     // restlen-restKappa is two diagonals
            result += numRestKappaVars();         // restKappa-restKappa just diagonal.
        }
    }
    return result;
}
...
template<typename Real_>
typename ElasticRod_T<Real_>::CSCMat ElasticRod_T<Real_>::hessianSparsityPattern(bool variableDesignParameters, Real_ val) const {
    CSCMat result;
    ...
    if (variableDesignParameters) {
        ...
        if (m_designParameterConfig.restKappa) {
            // The columns associated with the rest kappas contain the x-rk, theta-rk, rl-rk, and rk-rk blocks
            for (size_t vi = 0; vi < numRestKappaVars(); ++vi) {
                const size_t j = 3 * nv + ne + ne * m_designParameterConfig.restLen + vi;

                // Column for rk_vi has contributions from x_{vi}, x_{vi + 1}, x_{vi + 2}
                addIdxRange(3 * vi, 3 * (vi + 3));
                // Column for rk_vi has contributions from theta^{vi}, theta^{vi + 1}
                addIdxRange(3 * nv + vi, 3 * nv + vi + 2);
                if (m_designParameterConfig.restLen) {
                    // Column for rk_vi has contributions from rl^{vi}, rl^{vi + 1}
                    addIdxRange(3 * nv + ne + vi, 3 * nv + ne + vi + 2);
                }
                addIdx(j);
...
template<typename Real_>
void ElasticRod_T<Real_>::hessEnergyStretch(ElasticRod_T<Real_>::CSCMat &H, bool variableDesignParameters) const {
...
template<typename Real_>
void ElasticRod_T<Real_>::hessEnergyBend(ElasticRod_T<Real_>::CSCMat &H, bool variableDesignParameters) const {
    assert(H.symmetry_mode == CSCMat::SymmetryMode::UPPER_TRIANGLE);
    const size_t ndof = variableDesignParameters ? numExtendedDoF() : numDoF();
    UNUSED(ndof);
    assert((size_t(H.m) == ndof) && (size_t(H.n) == ndof));
    ...
    for (size_t i = 1; i < nv - 1; ++i) {
        ...
        for (size_t adj_edge = 0; adj_edge < 2; ++adj_edge) { // 0 ==> i - 1, 1 ==> i
            ...
            for (size_t k = 0; k < 2; ++k) {
                ...
                if (adj_edge == 0) d2_kappa_k_j_deim1_deim1 += (sign * 0.5) * (kb * d_kother_j.transpose() /* + d_kother_j * kb.transpose() */);
                if (adj_edge == 1) d2_kappa_k_j_dei_dei     += (sign * 0.5) * (kb * d_kother_j.transpose() /* + d_kother_j * kb.transpose() */);
     ...
     // perVertexHessian_x_x.template block<3, 3>(6, 3) =  d2E_deim1_dei.transpose() - d2E_dei_dei                                  ; // (x i + 1, x i    ) ==> (+ei, +ei-1), (+ei, -ei)
        ...
        /////////////////////////////////////////////
        for (size_t c1 = 0; c1 < 9; ++c1) H.addNZStrip(    x_offset,     x_offset + c1, perVertexHessian_x_x.col(c1).head(c1 + 1));
        for (size_t c1 = 0; c1 < 2; ++c1) H.addNZStrip(    x_offset, theta_offset + c1, perVertexHessian_x_theta.col(c1));
        for (size_t c1 = 0; c1 < 2; ++c1) H.addNZStrip(theta_offset, theta_offset + c1, perVertexHessian_theta_theta.col(c1).head(c1 + 1));

        /////////////////////////////////////////////
        // Design Parameter derivatives
        /////////////////////////////////////////////
        if (!variableDesignParameters) continue;
        ...
        /////////////////////////////////////////////
        // Rest kappa derivatives
        /////////////////////////////////////////////
        if (m_designParameterConfig.restKappa) {
            const size_t rk_offset = rl_offset + ne * m_designParameterConfig.restLen; //   index of the first rest kappa variable for the stencil

            const Vec2 ljbar_div_libar(m_restLen[i - 1] * 2 * inv_2libar,
                                       m_restLen[i    ] * 2 * inv_2libar);

            // d_2_Eb_d_2_rest_kappa
            // This term is the same for both Bergou2008 and Bergou2010.
            Real_ d_2_Eb_d_2_rk = 2 * B_div_2libar[0];

            // d_2_Eb_d_rest_kappa_d_theta
            Vec2 d_2_Eb_d_rk_d_theta;
            // The hessian term for Bergou2008 is scaled by ljbar / libar
            d_2_Eb_d_rk_d_theta = -B_div_2libar[0] * d_kappa_k_j_dtheta_j.row(0);
            if (m_bendingEnergyType == BendingEnergyType::Bergou2008)
                d_2_Eb_d_rk_d_theta = ljbar_div_libar.asDiagonal() * d_2_Eb_d_rk_d_theta;

            // d_2_Eb_d_rest_kappa_d_e
            // (Derivative with respect to edge vector).
            using M32d = Eigen::Matrix<Real_, 3, 2>;
            M32d d_2_Eb_d_rest_kappa_d_e = M32d::Zero();
            // The value of the rest kappa (0, j) and e_i hessian cross term. Scaled by ljbar / libar in Bergou 2008.
            for (size_t adj_edge = 0; adj_edge < 2; ++adj_edge) {
                Real_ scale = -B_div_2libar[0];
                if (m_bendingEnergyType == BendingEnergyType::Bergou2008)
                    scale *= ljbar_div_libar[adj_edge];

                d_2_Eb_d_rest_kappa_d_e.col(0) += scale * d_kappa_k_j_de_im1[0][adj_edge];
                d_2_Eb_d_rest_kappa_d_e.col(1) += scale * d_kappa_k_j_de_i  [0][adj_edge];
            }

            // d_2_Eb_d_rest_kappa_d_rest_length
            Vec2 d_2_Eb_d_rk_d_rl;
            if (m_bendingEnergyType == BendingEnergyType::Bergou2010) {
                d_2_Eb_d_rk_d_rl.array() = (2 * B_div_2libar[0] * inv_2libar) * (dc.kappa[i][0] - m_restKappa[i][0]);
            }
            else if (m_bendingEnergyType == BendingEnergyType::Bergou2008) {
                // Deviation of material curvature 0 from rest curvature on corners i - 1 and i, divided by libar
                Vec2 kappa_diff_div_libar = 2 * inv_2libar * (m_restKappa[i][0] - dc.per_corner_kappa[i].row(0).array());
                d_2_Eb_d_rk_d_rl = B_div_2libar[0] * (kappa_diff_div_libar.array() - kappa_diff_div_libar.dot(ljbar_div_libar));
            }

            size_t hint;
            hint = H.addNZStrip(x_offset + 0, rk_offset, -d_2_Eb_d_rest_kappa_d_e.col(0));
            hint = H.addNZStrip(x_offset + 3, rk_offset,  d_2_Eb_d_rest_kappa_d_e.col(0) - d_2_Eb_d_rest_kappa_d_e.col(1), hint);
            hint = H.addNZStrip(x_offset + 6, rk_offset,  d_2_Eb_d_rest_kappa_d_e.col(1),                                  hint);

            hint = H.addNZStrip(theta_offset, rk_offset, d_2_Eb_d_rk_d_theta, hint);
            hint = H.addNZStrip(   rl_offset, rk_offset, d_2_Eb_d_rk_d_rl,    hint);
            hint = H.addNZ     (   rk_offset, rk_offset, d_2_Eb_d_2_rk,       hint);
        }
    }
}
...
// Hessian ***evaluated assuming the source frame has been updated to the current frame***
template<typename Real_>
void ElasticRod_T<Real_>::hessEnergyTwist(ElasticRod_T<Real_>::CSCMat &H, bool variableDesignParameters) const {
    assert(H.symmetry_mode == CSCMat::SymmetryMode::UPPER_TRIANGLE);
    const size_t ndof = variableDesignParameters ? numExtendedDoF() : numDoF();
    assert((size_t(H.m) == ndof) && (size_t(H.n) == ndof));
    UNUSED(ndof);
    ...
    for (size_t i = 1; i < nv - 1; ++i) {
     ...
     // perVertexHessian_x_x.template block<3, 3>(6, 3) += d2E_dei_deim1 - d2E_dei_dei;                                               // (+ei, -ei), (+ei, +ei-1)
        ...
        for (size_t c = 0; c < 9; ++c) H.addNZStrip(x_offset,     x_offset + c, perVertexHessian_x_x.col(c).head(c + 1));
...
Eigen::MatrixX2d ElasticRod_T<Real_>::bendingStresses() const {
    ...
    for (size_t i = 1; i < nv - 1; ++i) {
        ...
        for (size_t adj_edge = 0; adj_edge < 2; ++adj_edge) {
            // WARNING: this may not be physically meaningful/accurate for curved rest states!
            Vec2 kappaDiff = dc.per_corner_kappa[i].col(adj_edge) - m_restKappa[i];
            sigma += m_restLen[i - 1 + adj_edge] * material(i - 1 + adj_edge).bendingStresses(stripAutoDiff(kappaDiff));
        }
...
Eigen::VectorXd ElasticRod_T<Real_>::twistingStresses() const {
    ...
    for (size_t i = 1; i < nv - 1; ++i) {
        Real_ libar2 = m_restLen[i - 1] + m_restLen[i];
        // WARNING: this may not be physically meaningful/accurate for twisted rest states!
...
template<typename Real_>
Eigen::VectorXd ElasticRod_T<Real_>::maxStresses(CrossSectionStressAnalysis::StressType type) const {
    const size_t nv = numVertices();
    Eigen::VectorXd result(nv);
    for (size_t i = 0; i < nv; ++i) {
        Real_ accum = 0.0, accum_len = 0.0;
        visitVertexStrains(i, [&](size_t j, Real_ tau, const Vec2 &curvatureNormal, Real_ eps_s) {
                // std::cout << "j: " << j << ", tau: " << tau << ", curvatureNormal: " << curvatureNormal.transpose() << ", eps_s: " << eps_s << std::endl;
                accum += m_restLen[j] * material(j).stressAnalysis().maxStress(type, tau, curvatureNormal, eps_s);
                accum_len += m_restLen[j];
            });
        result[i] = stripAutoDiff(accum / accum_len);
    }
    return result;
}

template<typename Real_>
Real_ ElasticRod_T<Real_>::surfaceStressLpNorm(CrossSectionStressAnalysis::StressType type, double p, bool takeRoot) const {
    Real_ result = 0.0;
    for (size_t i = 0; i < numVertices(); ++i) {
        visitVertexStrains(i, [&](size_t j, Real_ tau, const Vec2 &curvatureNormal, Real_ eps_s) {
                // `CrossSectionStressAnalysis::stressIntegral` computes the integral over the boundary curve. To obtain
                // a surface interval over the generalized open cylinder formed by extruding the cross-section by "l", we need to multiply
                // the integral by "l." We use the vertex's Voronoi region in the incident edge as the integration domain.
                using namespace std; // work-around for autodiff of `abs`
                result += 0.5 * m_restLen[j] * material(j).stressAnalysis().template stressIntegral</* SquaredMeasure = */ true>(type, tau, curvatureNormal, eps_s, [p](Real_ sigma) { return pow(Real_(abs(sigma)), Real_(p / 2.0)); } );
            });
    }
    return takeRoot ? pow(result, Real_(1.0 / p)) : result;
}

template<typename Real_>
typename ElasticRod_T<Real_>::Gradient ElasticRod_T<Real_>::gradSurfaceStressLpNorm(CrossSectionStressAnalysis::StressType type, double p, bool updatedSource, bool takeRoot) const {
    Gradient result(*this, /* include design parameters */ true);
    using namespace std; // work-around for autodiff of `abs`

    Real_ integral = 0.0;
    for (size_t i = 0; i < numVertices(); ++i) {
        visitVertexStrains(i, [&](size_t j, Real_ tau, const Vec2 &curvatureNormal, Real_ eps_s) {
                Real_ integrand = material(j).stressAnalysis().template stressIntegral</* SquaredMeasure = */ true>(type, tau, curvatureNormal, eps_s, [p](Real_ sigma) -> Real_ { return pow(Real_(abs(sigma)), Real_(p / 2.0)); } );
                integral += 0.5 * integrand * m_restLen[j];

                // Derivative due to changing integration domain
                if (result.variableRestLens)
                    result.gradRestLen(j) += 0.5 * integrand;

                Real_ grad_tau, grad_eps_s;
                Vec2 grad_curvatureNormal;
                // Warning: autodiff somehow breaks without the explicit return type in this lambda!
                auto integrandPrime = [p](Real_ sigma) -> Real_ { return p / 2.0 * pow(Real_(abs(sigma)), Real_(p / 2.0 - 1.0)); };
                material(j).stressAnalysis().template gradStressIntegral</* SquaredMeasure = */ true>(type, tau, curvatureNormal, eps_s, integrandPrime,
                                                                                                      grad_tau, grad_curvatureNormal, grad_eps_s);
                grad_tau             *= 0.5 * m_restLen[j];
                grad_eps_s           *= 0.5 * m_restLen[j];
                grad_curvatureNormal *= 0.5 * m_restLen[j];

                const auto &dc = deformedConfiguration();
                // grad_eps_s contribution
                // Recall eps_s formula:
                //      eps_s = (len[j] / m_restLen[j] - 1)
                M32d grad_e; // derivative with respect to edge vectors e_{i-1} and e_i
                size_t adj_edge = (j + 1) - i;
                grad_e.col(adj_edge) = (grad_eps_s / m_restLen[j]) * dc.tangent[j];
                grad_e.col(1 - adj_edge).setZero();
                if (result.variableRestLens)
                    result.gradRestLen(j) -= grad_eps_s * dc.len[j] / (m_restLen[j] * m_restLen[j]);

                // Twist and curvature are identically zero on the first/last vertex.
                const size_t nv = numVertices();
                if ((i > 0) && (i < numVertices() - 1)) {
                    Real_ libar2 = m_restLen[i - 1] + m_restLen[i];
                    // grad_tau contribution
                    // Recall tau formula:
                    //  tau = (dc.theta(i) - dc.theta(i - 1) + dc.referenceTwist[i] - m_restTwist[i]) / (0.5 * libar2);
                    M32d d_twist_d_e;
                    compute_d_twist_d_e<Real_>(updatedSource, dc, i, d_twist_d_e.col(0), d_twist_d_e.col(1));
                    grad_e += (grad_tau / (0.5 * libar2)) * d_twist_d_e;
                    if (result.variableRestLens) {
                        Real_ grad_restlen_contrib = -grad_tau * (dc.theta(i) - dc.theta(i - 1) + dc.referenceTwist[i] - m_restTwist[i]) / (0.5 * libar2 * libar2);
                        result.gradRestLen(i - 1) += grad_restlen_contrib;
                        result.gradRestLen(i    ) += grad_restlen_contrib;
                    }
                    result.gradTheta(i - 1) -= grad_tau / (0.5 * libar2);
                    result.gradTheta(    i) += grad_tau / (0.5 * libar2);

                    // grad_curvatureNormal contribution
                    // Recall curvature normal computation:
                    //   (dc.per_corner_kappa[i].col(adj_edge) - m_restKappa[i]) / (0.5 * libar2)
                    grad_e += (grad_curvatureNormal[0] / (0.5 * libar2)) * compute_d_kappa_i_k_j_de<Real_>(updatedSource, dc, i, /* k = */ 0, adj_edge);
                    grad_e += (grad_curvatureNormal[1] / (0.5 * libar2)) * compute_d_kappa_i_k_j_de<Real_>(updatedSource, dc, i, /* k = */ 1, adj_edge);
                    result.gradTheta(j) += (grad_curvatureNormal[0] * dc.per_corner_kappa[i](1, adj_edge) - grad_curvatureNormal[1] * dc.per_corner_kappa[i](0, adj_edge)) / (0.5 * libar2);

                    if (result.variableRestLens) {
                        Real_ grad_restlen_contrib = -grad_curvatureNormal.dot(dc.per_corner_kappa[i].col(adj_edge) - m_restKappa[i]) / (0.5 * libar2 * libar2);
                        result.gradRestLen(i - 1) += grad_restlen_contrib;
                        result.gradRestLen(i    ) += grad_restlen_contrib;
                    }
                    if (result.variableRestKappas) {
                        // The rest curvature variables control kappa_1, not kappa_2
                        result.gradRestKappa(i) -= grad_curvatureNormal[0] / (0.5 * libar2);
                    }
                }

                if (i !=      0) result.gradPos(i - 1) -= grad_e.col(0);
                /*            */ result.gradPos(i    ) += grad_e.col(0) - grad_e.col(1);
                if (i != nv - 1) result.gradPos(i + 1) += grad_e.col(1);

                if (std::isnan(stripAutoDiff(result.squaredNorm()))) {
                    std::cout << std::endl << std::endl;
                    std::cout << "nans encountered: " << result.transpose() << std::endl;
                    std::cout << "tau:             " << tau                         << std::endl;
                    std::cout << "curvatureNormal: " << curvatureNormal.transpose() << std::endl;
                    std::cout << "eps_s:           " << eps_s                       << std::endl;
                    std::cout << "grad_tau:             " << grad_tau                         << std::endl;
                    std::cout << "grad_curvatureNormal: " << grad_curvatureNormal.transpose() << std::endl;
                    std::cout << "grad_eps_s:           " << grad_eps_s                       << std::endl;
    ...
    if (takeRoot)
        result *= (1 / p) * pow(integral, Real_(1.0 / p - 1.0));
...
ElasticRod_T<Real_>::ElasticRod_T(const ElasticRod_T<Real2> &r) {
    ...
    setDesignParameterConfig(r.getDesignParameterConfig().restLen,
                             r.getDesignParameterConfig().restKappa);
