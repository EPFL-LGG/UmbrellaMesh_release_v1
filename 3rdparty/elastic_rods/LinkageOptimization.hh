...
enum class OptEnergyType { Full, ElasticBase, ElasticDeployed, Target, Regularization, Smoothing, ContactForce };
enum class PredictionOrder : int { Zero = 0, One = 1, Two = 2};

template<template<typename> class Object>
struct LinkageOptimization {
    LinkageOptimization(Object<Real> &baseLinkage, const NewtonOptimizerOptions &eopts, Real E0, Real l0, Real rl0)
    : m_equilibrium_options(eopts), m_numParams(baseLinkage.numDesignParams()), m_E0(E0), m_l0(l0), m_rl0(rl0), m_base(baseLinkage), m_linesearch_base(baseLinkage) {}
    ...
    const Eigen::VectorXd &params() const { return m_base.getDesignParameters(); }
    ...
    Real J(const Eigen::Ref<const Eigen::VectorXd> &params, OptEnergyType opt_eType = OptEnergyType::Full) {
        std::cout << "eval at dist " << (m_base.getDesignParameters() - params).norm() << std::endl;
        std::cout << "eval at linesearch dist " << (m_linesearch_base.getDesignParameters() - params).norm() << std::endl;
    ...
    void setRestKappaSmoothingWeight(Real val)     { objective.get("RestCurvatureSmoothing").setWeight(val); }
    void setContactForceWeight(Real val)     { objective.get("ContactForce").setWeight(val); }
    ...
    Real getRestKappaSmoothingWeight()     const { return objective.get("RestCurvatureSmoothing").getWeight(); }
    Real getContactForceWeight() const { return objective.get("ContactForce").getWeight(); }
...
protected:
    ...
    virtual bool m_updateAdjointState(const Eigen::Ref<const Eigen::VectorXd> &params) = 0;
    ...
    Real m_E0 = 1.0, m_l0 = 1.0, m_rl0 = 1.0;
    Real beta = 1.0;
