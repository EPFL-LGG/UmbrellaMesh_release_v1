...
struct OptKnitroProblem : public KnitroProblem<OptKnitroProblem<Object>> {
    ...
    OptKnitroProblem(LinkageOptimization<Object> &lopt, bool applyAngleConstraint, bool applyFlatnessConstraint, double minRestLen)
        : Base(lopt.numParams(), /* num constraints */ int(applyAngleConstraint) + int(applyFlatnessConstraint)), m_lopt(lopt),
    ...
    double evalFC(const double *x,
        ...
        if (m_applyAngleConstraint || m_applyFlatnessConstraint) {
	        ...
	        Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>> constraintJacobian(jac, numConstraints(), np);
...
private:
    ...
    bool m_applyAngleConstraint, m_applyFlatnessConstraint;
};
...
int LinkageOptimization<Object>::optimize(OptAlgorithm alg, size_t num_steps,
    ...
    std::vector<Real> x_init(numParams());
    Eigen::Map<Eigen::VectorXd>(x_init.data(), x_init.size()) = getLinesearchBaseLinkage().getDesignParameters();
