...
struct EquilibriumProblem : NewtonProblem {
    ...
    virtual void writeDebugFiles(const std::string &errorName) const override {
        ...
        std::vector<bool> shouldRemove(H.m, false);
        for (size_t i : fixedVars()) shouldRemove[i] = true;

        H.rowColRemoval([&](size_t i) { return shouldRemove[i]; });
        M.rowColRemoval([&](size_t i) { return shouldRemove[i]; });
...
template<typename Object>
std::unique_ptr<EquilibriumProblem<Object>> equilibrium_problem(Object &obj, const std::vector<size_t> &fixedVars = std::vector<size_t>()) {
    auto problem = std::make_unique<EquilibriumProblem<Object>>(obj);
    problem->addFixedVariables(fixedVars);
    return problem;
}

template<typename Object>
std::unique_ptr<NewtonOptimizer> get_equilibrium_optimizer(Object &obj, Real targetAverageAngle, const std::vector<size_t> &fixedVars = std::vector<size_t>(), CallbackFunction customCallback = nullptr) {
...
template<typename Object>
std::unique_ptr<NewtonOptimizer> get_equilibrium_optimizer(Object &obj, const std::vector<size_t> &fixedVars = std::vector<size_t>(), CallbackFunction customCallback = nullptr) {
    auto problem = equilibrium_problem(obj, fixedVars);
    problem->setCustomIterationCallback(customCallback);
    return std::make_unique<NewtonOptimizer>(std::move(problem));
}
...
template<typename Object>
ConvergenceReport
compute_equilibrium(Object &obj, Real targetAverageAngle, const NewtonOptimizerOptions &opts = NewtonOptimizerOptions(), const std::vector<size_t> &fixedVars = std::vector<size_t>(), CallbackFunction customCallback = nullptr) {
    auto opt = get_equilibrium_optimizer(obj, targetAverageAngle, fixedVars, customCallback);
    opt->options = opts;
    return opt->optimize();
}
...
template<typename Object>
ConvergenceReport
compute_equilibrium(Object &obj, const NewtonOptimizerOptions &opts = NewtonOptimizerOptions(), const std::vector<size_t> &fixedVars = std::vector<size_t>(), CallbackFunction customCallback = nullptr) {
    auto opt = get_equilibrium_optimizer(obj, fixedVars, customCallback);
    opt->options = opts;
    return opt->optimize();
}
...
template<typename Object>
ConvergenceReport
compute_equilibrium(Object &obj, const Eigen::VectorXd &externalForces, const NewtonOptimizerOptions &opts = NewtonOptimizerOptions(), const std::vector<size_t> &fixedVars = std::vector<size_t>(), CallbackFunction customCallback = nullptr) {
    auto problem = equilibrium_problem(obj, fixedVars);
