...
struct KnitroProblem : public knitro::KTRProblem {
    ...
    int evalHessVec(const std::vector<double> &x, double sigma, const std::vector<double> &lambda,
                    std::vector<double> &vec) {
        return derived().evalHessianVector(x.data(), sigma, lambda.data(), vec.data(), vec.data());
    }
