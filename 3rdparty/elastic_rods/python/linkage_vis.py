...
class LinkageViewerWithSurface(LinkageViewer):
    ...
    def __init__(self, straight_linkage, target_surf, *args, **kwargs):
        if (isinstance(target_surf, str)):
            import mesh
            target_surf = mesh.Mesh(target_surf)

        super().__init__(straight_linkage, *args, **kwargs)
        self.surfView = tri_mesh_viewer.TriMeshViewer(target_surf, superView=self)

        self.averagedMaterialFrames = True
        self.averagedCrossSections  = True

        self.viewOptions = {self.ViewType.LINKAGE: self.ViewOption(True, 'green'),
                            self.ViewType.SURFACE: self.ViewOption(False, 'lightgray')}
