...
def open_umbrella(umbrella, jointIdx, fullAngle, numSteps, view = None,
                 ...
                 fixedVars = [], 
                 scalarField = None):
    j = umbrella.joint(jointIdx)
    jdo = umbrella.dofOffsetForJoint(jointIdx)
    if (useTargetAngleConstraint):
    ...
    stepper = AngleStepper(useTargetAngleConstraint, umbrella, jointIdx, fullAngle, numSteps)

    def reportIterate(it):
        print("\t".join(map(str, [stepper.joint.alpha, umbrella.energy()]
                               + [umbrella.energy(elasticEnergyType = t) for t in EnergyType.__members__.values()])))
    convergenceReports = []
    actuationForces = []
    average_angles = []
    ...
    for it in range(1, numSteps + 1):
        ...
        if (iterationCallback is not None): iterationCallback(it - 1, umbrella)
        stepper.step()
        perturb_joints(umbrella, zPerturbationEpsilon)

        tgtAngle = stepper.currentAngle if useTargetAngleConstraint else elastic_rods.TARGET_ANGLE_NONE
        print("target angle: ", tgtAngle)
        r = equilibriumSolver(tgtAngle, umbrella, opts, fixedVars)
        # pickle.dump(umbrella, open('open_post_step_{}.pkl'.format(it), 'wb'))

        convergenceReports.append(r)
        actuationForces.append(umbrella.gradient()[umbrella.dofOffsetForJoint(jointIdx) + 6])
        average_angles.append(umbrella.averageJointAngle)

        if (view is not None):
            if (scalarField is not None):
                view.update(False, scalarField=scalarField)
            else:
                view.update(False)

        if (outPathFormat is not None):
            umbrella.saveVisualizationGeometry(outPathFormat.format(it), averagedMaterialFrames=True)
    ...
    opts.niter = 1000;
    tgtAngle = stepper.currentAngle if useTargetAngleConstraint else elastic_rods.TARGET_ANGLE_NONE
    r = finalEquilibriumSolver(tgtAngle, umbrella, opts, fixedVars)
    convergenceReports.append(r)

    if (iterationCallback is not None):
        iterationCallback(len(convergenceReports) - 1, umbrella)
